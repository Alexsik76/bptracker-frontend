...
export function useKpi(source: MaybeRefOrGetter<Measurement[]>) {
  return computed((): KpiData | null => {
    ...
    if (last7.length >= 3 && prev7.length >= 3) {
      ...
      const curDia = avgNum(last7.map(m => m.dia))
      const prevSys = avgNum(prev7.map(m => m.sys))
      const prevDia = avgNum(prev7.map(m => m.dia))
      if (curSys !== null && prevSys !== null) deltaSys = curSys - prevSys
      if (curDia !== null && prevDia !== null) deltaDia = curDia - prevDia
    }
    ...
    return {
      ...
      normalShare, normalCount, totalLast7,
      deltaSys, deltaDia,
      deltaIcon, deltaColor,
    }
  })
}
